...
// Stage 4 — Out of angles (batch rows)
...
/* ============================================================
 * PlotWise — Managed Campaigns · The Loop (timeline-driven)
 * Mirrors the homepage Problem timeline system, mapped onto
 * the managed-campaign narrative: find a winner → it fatigues
 * → you clone it → you run out of angles.
 * ============================================================ */

// Brand tokens (raw values to match the canvas system exactly)
const MINT = "#91CEB0";
const TEAL = "#207770";
const CREAM = "#F8FAF9";
const RED = "#C45040";
...
const SCENES_META = [
  { id: 0, num: "01", label: "You find a winner", start: 0, end: 9 },
  { id: 1, num: "02", label: "Then it dies", start: 9, end: 18 },
  { id: 2, num: "03", label: "So you clone it", start: 18, end: 27 },
  { id: 3, num: "04", label: "You run out of angles", start: 27, end: 36 },
] as const;
...
function StageLabel({ n, op }: { n: number; op: number }) {
  return (
    <div
      style={{
        position: "absolute",
...
/* ══════════════════════════════════════════════════════════
 * Mobile carousel — swipe / drag through the 4 stages
 * ══════════════════════════════════════════════════════════ */
function MobileLoopCarousel() {
  ...
  const cards: {
    ...
    {
      num: "01",
      title: "You find a winner.",
      stats: [
        { value: "1 of 12", label: "Creatives → winner", highlight: true },
        { value: "4.8×", label: "Peak ROAS", highlight: true },
        { value: "−38%", label: "CAC drop" },
        { value: "2–3 wks", label: "Of outperformance" },
      ],
      bullets: [
        "One creative breaks through. CAC drops, you scale spend.",
        "For two or three weeks the account beats the whole quarter.",
        "It feels like you finally cracked it.",
      ],
      quote: "“For a few weeks, the winner carries the entire account.”",
      img: IMGS.creator1,
    },
    {
      num: "02",
      title: "Then it starts dying.",
      stats: [
        { value: "Day 1–7", label: "Scaling" },
        { value: "Day 7–14", label: "Peak ROAS", highlight: true },
        { value: "Day 14+", label: "Fatigue sets in", danger: true },
      ],
      bullets: [
        "Beauty creative fatigues in 2 to 3 weeks at scale.",
        "Frequency climbs, CPA creeps, the edge quietly erodes.",
        "Your account tells you it won. It never tells you why.",
      ],
      quote: "“The winner carrying your month quietly loses its edge.”",
      img: IMGS.serum,
      quoteBorder: RED,
    },
    {
      num: "03",
      title: "So you clone it.",
      stats: [
        { value: "0 of 3", label: "Clones beat the original", danger: true },
        { value: "Faster", label: "Fatigue, not slower", danger: true },
      ],
      bullets: [
        "New creator, new hook, new b-roll — same angle underneath.",
        "Your audience already saw the idea. Fatigue accelerates.",
        "A clone isn't a new ad. It's the same ad in a new outfit.",
      ],
      quote: "“Surface changes don't reset fatigue. They speed it up.”",
      img: IMGS.cream,
    },
    {
      num: "04",
      title: "You run out of angles — not videos.",
      stats: [
        { value: "1", label: "Angle, recycled all quarter", danger: true },
        { value: "0", label: "New ground discovered", danger: true },
      ],
      bullets: [
        "Cloning lets you avoid the hard part: the next angle.",
        "The next barrier to break. The next belief to shift.",
        "Your winner told you nothing about where to go next.",
      ],
      quote: "“Brands don't run out of videos. They run out of angles.”",
      img: p4_11,
      quoteBorder: RED,
    },
  ];
...
function Scene1({ t }: { t: number }) {
  ...
  return (
    <SceneWrapper scOp={scOp}>
      ...
      <StageLabel n={1} op={lOp} />
      <H lines={["You find a", "winner."]} op={hOp} dy={hY} />
      ...
      >
        <StatPill value="1 of 12" label="Creatives → winner" highlight />
        <StatPill value="4.8×" label="Peak ROAS" highlight />
        <StatPill value="−38%" label="CAC drop" />
        <StatPill value="2–3 wks" label="Of outperformance" />
      </div>

      <Bullets
        items={[
          "One creative breaks through. CAC drops, you scale spend.",
          "For two or three weeks it beats the whole quarter.",
          "It feels like you finally cracked it.",
        ]}
        op={bOp}
        top={560}
      ...
      <Quote
        text={'"For a few weeks, the winner carries the entire account."'}
        op={iOp}
      />
    </SceneWrapper>
  );
...
function Scene2({ t }: { t: number }) {
  ...
  return (
    <SceneWrapper scOp={scOp}>
      ...
      <StageLabel n={2} op={lOp} />
      <H lines={["Then it starts", "dying."]} op={hOp} dy={hY} />
      ...
      <Bullets
        items={[
          "Beauty creative fatigues in 2 to 3 weeks at scale.",
          "Frequency climbs, CPA creeps, the edge erodes.",
          "Your account tells you it won. Never why.",
        ]}
        op={bOp}
        top={540}
      />
      ...
      <Quote
        text={
          '"The winner carrying your month<br/>quietly loses its edge."'
        }
        op={iOp}
        border={RED}
      />
...
/* ══════════════════════════════════════════════════════════
 * SCENE 3 · So you clone it
 * ══════════════════════════════════════════════════════════ */
const CLONES = [
  ["Original", "Clone v2", "Clone v3"],
  ["Original", "Clone v2", "Clone v3"],
  ["Original", "Clone v2", "Clone v3"],
];
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "13px 14px 15px" }}>
        ...
        >
          Same hook. Same promise. Same angle.
        </div>
        <div
          style={{
            fontFamily: SANS,
        ...
        >
          New creator, new b-roll — your audience has seen it.
        </div>
        <div
          style={{
            display: "inline-flex",
...
function Scene3({ t }: { t: number }) {
  ...
  return (
    <SceneWrapper scOp={scOp}>
      ...
      <StageLabel n={3} op={lOp} />
      <H lines={["So you clone", "the winner."]} op={hOp} dy={hY} />

      <Bullets
        items={[
          "New creator, new hook — the same angle underneath.",
          "Your audience already saw the idea. Fatigue speeds up.",
          "A clone isn't a new ad. It's a new outfit.",
        ]}
        op={bOp}
        top={380}
      />
      ...
      <Quote
        text={
          '"Surface changes don\'t reset fatigue.<br/>They speed it up."'
        }
        op={iOp}
        border={RED}
      />
...
function Scene4({ t }: { t: number }) {
  ...
  return (
    <SceneWrapper scOp={scOp}>
      ...
      <StageLabel n={4} op={lOp} />
      <H
        lines={["You run out of", "angles —", "not videos."]}
        op={hOp}
        dy={hY}
      />
      ...
      >
        <FRow
          ...
          color={TEAL}
          tiles={[
            { img: p4_11, title: "Glass Skin Routine", type: "TikTok · Original", creator: "@danielllestrada" },
            { img: p4_12, title: "Morning GRWM", type: "Instagram · Edit", creator: "@meilinbeauty" },
            { img: p4_13, title: "Skincare Check", type: "TikTok · Edit", creator: "@amiraglows" },
          ]}
          d={2.0}
          lt={lt}
          gap
        ...
        <FRow
          ...
          color={MINT}
          tiles={[
            { img: p4_14, title: "Glass Skin, Again", type: "YouTube · Re-cut", creator: "@richualofficial" },
            { img: p4_15, title: "Same 5-Step", type: "Instagram · Re-cut", creator: "@skincarelab" },
            { img: p4_16, title: "Before & After", type: "TikTok · Re-cut", creator: "@glowjourney" },
          ]}
          d={3.4}
          lt={lt}
          gap
        ...
        <FRow
          label="Batch 03 — Q3"
          sub="More of the same"
          color="rgba(32,119,112,0.5)"
          tiles={[
            { img: p4_17, title: "Honest Review", type: "TikTok · Re-cut", creator: "@cleoritual" },
            { img: p4_18, title: "Testimonial", type: "Instagram · Re-cut", creator: "@skinbyaria" },
            { img: p4_19, title: "Last Chance", type: "Meta · Re-cut", creator: "@novaskinco" },
          ]}
          d={4.8}
          lt={lt}
          gap={false}
      ...
      <Quote
        text={
          '"Brands don\'t run out of videos.<br/>They run out of angles."'
        }
        op={iOp}
        border={RED}
      />
...
/* ══════════════════════════════════════════════════════════
 * Public component: ManagedProblemTimeline
 * ══════════════════════════════════════════════════════════ */
export function ManagedProblemTimeline() {
  ...
  if (isMobile) return <MobileLoopCarousel />;
