...
<head>
  ...
  <title>New Managed Campaign Application</title>
</head>
            ...
            <td style="background:#ffffff;border-radius:0 0 16px 16px;overflow:hidden;">
              ...
              <div style="padding:24px 40px;background:#f8fbfa;border-top:1px solid #e8efed;">
                <p style="margin:0;font-size:12px;color:#9aafac;text-align:center;">
                  Managed Campaign form · ${new Date().toLocaleString("en-US", { dateStyle: "long", timeStyle: "short" })}
                </p>
...
const SHEET_WEBHOOK = process.env.MANAGED_SHEET_WEBHOOK ?? process.env.CONTACT_SHEET_WEBHOOK ?? "";
  ...
  if (SHEET_WEBHOOK) {
    try {
      const sheetRes = await fetch(SHEET_WEBHOOK, {
        ...
        body:    JSON.stringify({ form: "managed", answers, otherText: otherText ?? {} }),
      });
      const sheetData = await sheetRes.json();
      console.log("Managed sheet result:", JSON.stringify(sheetData));
    } catch (err) {
      console.error("Managed sheet error (non-fatal):", err);
    }
  }
  ...
  if (process.env.RESEND_API_KEY) {
    try {
      ...
      const result = await resend.emails.send({
        ...
        replyTo: replyEmail,
        subject: `Managed Campaign Application — ${brandName}`,
