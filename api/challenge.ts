...
const LABELS: Record<string, string> = {
  ...
  paidAds:       "Running Paid Ads",
  adAccount:     "TikTok Ad Account",
  contentVolume: "Monthly Content Volume",
  goal:          "Campaign Goal",
  name:          "Contact Name",
  email:         "Email Address",
};
...
<head>
  ...
  <title>New Challenge Campaign Application</title>
</head>
          ...
          <tr>
            <td style="background:linear-gradient(135deg,#0F4F4A 0%,#207770 55%,#2C8A82 100%);border-radius:16px 16px 0 0;padding:36px 40px 32px;">
              ...
              <div style="font-size:13px;color:rgba(255,255,255,0.65);letter-spacing:0.05em;text-transform:uppercase;">
                Challenge Campaign Application
              </div>
            ...
            <td style="background:#ffffff;border-radius:0 0 16px 16px;overflow:hidden;">
              ...
              <div style="padding:24px 40px;background:#f8fbfa;border-top:1px solid #e8efed;">
                <p style="margin:0;font-size:12px;color:#9aafac;text-align:center;">
                  Challenge Campaign form · ${new Date().toLocaleString("en-US", { dateStyle: "long", timeStyle: "short" })}
                </p>
...
const SHEET_WEBHOOK = process.env.CHALLENGE_SHEET_WEBHOOK ?? process.env.CONTACT_SHEET_WEBHOOK ?? "";
  ...
  if (SHEET_WEBHOOK) {
    try {
      const sheetRes = await fetch(SHEET_WEBHOOK, {
        ...
        body:    JSON.stringify({ form: "challenge", answers, otherText: otherText ?? {} }),
      });
      const sheetData = await sheetRes.json();
      console.log("Challenge sheet result:", JSON.stringify(sheetData));
    } catch (err) {
      console.error("Challenge sheet error (non-fatal):", err);
    }
  }
  ...
  if (process.env.RESEND_API_KEY) {
    try {
      ...
      const result = await resend.emails.send({
        ...
        replyTo: replyEmail,
        subject: `Challenge Campaign Application — ${brandName}`,
